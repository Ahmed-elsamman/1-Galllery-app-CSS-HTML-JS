let aboutGallery = JSON.parse(localStorage.getItem('aboutGallery')) || [];
...
function deleteProduct(id) {
    aboutGallery = aboutGallery.filter(product => product.id !== id);
    localStorage.setItem('aboutGallery', JSON.stringify(aboutGallery));
    displayProducts(aboutGallery);
}
...
function editProduct(id) {
    const product = aboutGallery.find(product => product.id === id);
    console.log('id', id);
...
productList.addEventListener('click', function (e) {
    ...
    currentIndexOfProduct = aboutGallery.findIndex(product => product.image === e.target.getAttribute('src'));
});

function changeSliderImage(direction) {
    if (!aboutGallery.length) return;

    if (direction === 'right') {
        currentIndexOfProduct = (currentIndexOfProduct + 1) % aboutGallery.length;
        console.log('currentIndexOfProduct',  (currentIndexOfProduct + 1) % aboutGallery.length);
        
    } else if (direction === 'left') {
        currentIndexOfProduct = (currentIndexOfProduct - 1 + aboutGallery.length) % aboutGallery.length;
        console.log('currentIndexOfProduct',  (currentIndexOfProduct - 1) % aboutGallery.length);
    }

    const imageSrc = aboutGallery[currentIndexOfProduct].image || './images/no.jpg';
    sliderOfProducts.style.backgroundImage = `url('${imageSrc}')`;
}
